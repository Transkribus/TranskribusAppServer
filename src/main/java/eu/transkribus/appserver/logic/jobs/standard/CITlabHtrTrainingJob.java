...
public class CITlabHtrTrainingJob extends ATrpJobRunnable {
	
	private static final Logger logger = LoggerFactory.getLogger(CITlabHtrTrainingJob.class);
	private final CitLabHtrTrainConfig config;
	private final Integer nrOfThreads;
	...
	protected CITlabHtrTrainingJob(TrpJobStatus job) throws JAXBException {
		...
		try {
			config = JaxbUtils.unmarshal(objectStr, CitLabHtrTrainConfig.class, 
					DocumentSelectionDescriptor.class, PageDescriptor.class);
		} catch (JAXBException e) {
			setJobStatusFailed("Could not load Config!", e);
			throw e;
		}
		nrOfThreads = getIntProperty(JobConst.PROP_NR_OF_THREADS);
	}
	...
	public void doProcess() throws JobCanceledException {
		...
		//create gt document
		try {
			gt = docMan.duplicateDocument("TRAIN_CITlab_" + config.getModelName(), job.getUserId(), job.getUserName(), config.getTrain());
		...
		if(config.getTest() != null && !config.getTest().isEmpty()) {
			...
			try {
				testGt = docMan.duplicateDocument("TEST_CITlab_" + config.getModelName(), 
						job.getUserId(), job.getUserName(), 
						config.getTest());
