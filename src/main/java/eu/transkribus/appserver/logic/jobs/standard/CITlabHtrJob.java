...
public class CITlabHtrJob extends ATrpJobRunnable {
	
	private static final Logger logger = LoggerFactory.getLogger(CITlabHtrJob.class);
...
//	private String state = JobConst.STATE_HTR;
	...
	public void doProcess() throws JobCanceledException {
		...
		TrpHtr htr;
		try {
			htr = hMan.getHtrById(modelId);
		} catch (Exception e) {
			setJobStatusFailed("Could not retrieve HTR from DB!", e);
			return;
		}
		
		setJobStatusProgress("Preparing model...");
		
		final File net = HtrManager.getCITlabSprnnFile(htr);
		if(!net.isFile()){
...
//
		...
		//FIXME
		final String toolName = "CITlab HTR: " + htr.getName();
